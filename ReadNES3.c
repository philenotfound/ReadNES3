 ...
 * ReadNES3.c
 *
 * Provides functions for splitting .NES homebrew ROMs into hardware binaries.
 *
 * Copyright (C) 2011-2016 Aaron Bottegal
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 *
 * Original work Copyright (C) 2011-2016 Aaaron Bottegal
 */
...
char Buffer[16384];
unsigned char BaseROMFileName[1024];
...
//File header binary data.
    ...
    if (HeaderTest[0] != 'N' || HeaderTest[1] != 'E' || HeaderTest[2] != 'S'
            || HeaderTest[3] != 0x1A) {
        fclose(ROMFile);
        printf("\n\nBad header, aborting.\n\n");
        return 0;
    ...
    for (RepeatLoop = 0; RepeatLoop < ProgramRepeat; RepeatLoop++) {
        for (DataTransferLoop = 0; DataTransferLoop < ProgramBanks;
                DataTransferLoop++) {
    ...
    for (RepeatLoop = 0; RepeatLoop < CharacterRepeat; RepeatLoop++) {
        for (DataTransferLoop = 0; DataTransferLoop < CharacterBanks;
                DataTransferLoop++) {
